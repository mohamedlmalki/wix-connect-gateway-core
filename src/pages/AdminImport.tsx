...
const AdminImport = () => {
    ...
    const [searchResults, setSearchResults] = useState<SiteMember[]>([]);
    ...
    const handleSearch = async () => {
        ...
        if (!searchQuery) {
            toast.warning("Please enter a search query (e.g., a name or email).");
            return;
        }
        setIsSearching(true);
        setSelectedMembers([]);
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            const responseText = await response.text();
            if (!responseText) {
                throw new Error("Received an empty response from the server. Check backend logs.");
            }
            const result = JSON.parse(responseText);
        ...
        <div className="min-h-screen bg-gradient-subtle">
            ...
            <div className="container mx-auto px-4 pt-24 pb-12">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <Card className="bg-gradient-card shadow-card border-primary/10"><CardHeader><CardTitle className="flex items-center gap-2"><Building className="h-5 w-5" />Site Selection</CardTitle><CardDescription>Choose a site to manage its members or import new ones.</CardDescription></CardHeader><CardContent>{isLoadingSites ? <p>Loading sites...</p> : (<div className="max-w-md"><Select value={selectedSite} onValueChange={setSelectedSite} disabled={sites.length === 0}><SelectTrigger><SelectValue placeholder="No sites added yet..." /></SelectTrigger><SelectContent>{sites.map((site) => (site && <SelectItem key={site._id} value={site.siteId}>{site.siteName}</SelectItem>))}</SelectContent></Select></div>)}</CardContent></Card>
                                                ...
                                                <TableHead className="w-[50px]">
                                                    <Checkbox
                                                        ...
                                                        onCheckedChange={(checked) => {
                                                            setSelectedMembers(checked ? searchResults.map(m => m.id) : []);
                                                        }}
                                                    />
                                                </TableHead>
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            ...
                                            <AlertDialogDescription>
                                                This will permanently delete the selected {selectedMembers.length} member(s) from the site. This action cannot be undone.
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
